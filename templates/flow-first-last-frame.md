...
```text
Begin exactly from the supplied first frame. Preserve [identity, wardrobe, props, location, palette and lighting anchors]. Over [duration], [chronological transition path with subject/environment motion]. The camera [coherent movement path]. [Audio progression]. Arrive naturally and precisely at the supplied last frame, with [final pose, gaze, prop state and camera state].
```
